 ...
 *   环境变量可覆盖：AI_BASE_URL / AI_API_KEY / AI_MODEL / PARALLEL / DICT_DB / ANNOTATE_STATE /
...
const VALUE_OPTS = new Set([
  '--parallel', '--base-url', '--api-key', '--model', '--input',
...
function appendJsonl(ann, wordText) {
  try {
    ...
    fs.appendFileSync(CONFIG.output, JSON.stringify({ word: wordText, ...ann }) + '\n', 'utf8');
  } catch (e) {
...
async function callChat(messages, useJsonMode) {
...
async function runAi(db, words) {
  ...
  log(null, `\n待标注 ${pending.length} 项（并发 ${CONFIG.concurrency}，完成一项立刻领下一项）:\n` +
