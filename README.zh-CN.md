...
| 🧠 **背词模式** | 按 IELTS 频率加权的自适应间隔重复,含干扰项引擎、当堂检测与次日四选一复习。 |
| 🤖 **AI 语感标注** | AI 逐词生成的语感标注——感情色彩、语体、精细语意、避坑提示,以及可朗读、自动译文的常见搭配。 |
| 📅 **每日探索** | 完全离线的每日探索流,按 **5:3:2** 配比混合核心新词 / 词根拓展 / 高频过渡词;卡面带例句、助记与真题热度,只做阅读预热,复习与测验留在背词页。 |
...
4. **探索 · Explore** —— 每日探索流(只阅读、不测验)

App 启动默认打开**词典**标签页。导航是响应式的:窄窗口用底部导航栏,平板 / 大屏用侧边导航栏;支持 Android 系统返回 / 手势导航与词详情滑页切换。外壳维护真实的访问历史栈,系统返回回到你真正来源的标签页(含跨标签跳词,如 探索 → 词详情 → 返回探索流),并通过 saveable state holder 保存每个标签页的滚动 / 搜索 / 详情状态。
...
- **当堂检测**:新词卡点「我已背会」后立刻做一次四选一提取练习——答对才写入复习管线并计入今日进度,答错可重答,实在不会可「暂时跳过」退回学习队列(不入库、不计进度)。当堂检测只是入门门槛,不推进 ASR 阶梯,明日首轮复习照常安排。
...
**探索**标签页构建**完全离线**的每日探索流(方案A 定位分离:本页只做轻度阅读 / 预热,复习与测验权交还背词页):
...
- **阅读上下文**:当前卡带例句(可朗读、自动译文)、助记与真题热度柱状图;本页全程不出四选一,提取练习交给背词页。
- **实时学习状态**:卡面绑定 `study.db` 的真实状态(未学 / 学习中 / 复习中 / 已掌握),在词典页把词标为已掌握会通过 Room Flow 即时从探索流剔除。
- **写回复习管线**:「纳入复习计划」立刻入库(明日首轮复习),「已掌握」直接消灭该词,背词引擎随之跳过。
- 进度按天持久化,跨启动保持稳定。
...
- **探索页定位分离(方案A + 方案C)**:推荐标签改名为**探索**并换用 Explore 图标,消除「两个入口都在问今天学什么」的心智混乱;探索卡新增例句 / 助记 / 真题热度,并绑定 `study.db` 实时学习状态(词典页标为已掌握会即时从流里剔除);背词页新增**当堂检测**——新词卡点「我已背会」后立即四选一,答对才入库计分,答错可重答、可暂时跳过,阅读输入归探索页、提取练习归背词页。
