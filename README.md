...
> **Note:** `dict.db` is generated by CI and kept out of Git (the repo intentionally does not carry a 50–70 MB binary). **You must run the data generation step before building locally**; the code never silently substitutes sample or fabricated data.

---

## Data Pipeline

The repo intentionally contains **no fabricated dictionary data or unauthorized copies of source data**. Production assets are generated from user-provided authorized exports:

```bash
# Local conversion (from an authorized export file)
python scripts/convert_dictionary.py /path/to/authorized-export.json app/src/main/assets/dict.db \
  --expected-word-count 49213
```

The converter accepts JSON / JSONL, base85-wrapped JSON, and Brotli-compressed JSON. It produces normalized `words`, `derived_terms`, `roots`, `sentences`, `word_sentence_links`, and `heatmap_entries` tables, plus a `word_search` English FTS table; it verifies record counts and emits per-table counts (JSON). The converter does **not** scrape the web — the data source and schema must be explicit and reproducible.

Generate and validate in full from an authorized site:
...
`dict.db` is generated and validated in CI and uploaded only as a build/release artifact — it never enters the Git repository. It is kept **compressed** (no `noCompress` override) — serving `dict.db` uncompressed would roughly double the APK size — and ships via AAB per-device delivery.
